#!/usr/bin/env python3
"""B1c 止血 — 基于 figure_audit.json 清洗 planner_cache 的错位图。

读 ``experiments/results/figure_audit.json``(由 audit_figures.py 产出),
把所有 status=="broken" 的图从 planner_cache 的 panel 引用里摘掉:该 panel
退化为 text_only(figure_id/figure/figure_caption 清空),并在 figures 字典里
给这些图打 ``audit_status: broken`` 标记(便于 future 的图复用率指标过滤)。
status=="weak" 的图保留不动。
...
def _broken_by_paper(audit_path: Path) -> Dict[str, Set[str]]:
    audit = json.loads(audit_path.read_text(encoding="utf-8"))
    out: Dict[str, Set[str]] = {}
    for r in audit.get("results", []):
        for f in r.get("figures", []):
            if f.get("status") == "broken":
                out.setdefault(r["paper"], set()).add(f["figure_id"])
    return out


def clean_doc(doc: dict, broken: Set[str]) -> int:
    """Demote panels referencing a broken figure to text_only. Returns #panels changed."""
    n = 0
    for panel in doc.get("panels", []) or []:
        if str(panel.get("figure_id") or "") in broken:
            panel["figure_id"] = ""
            panel["figure"] = None
            panel["figure_caption"] = ""
            panel["layout_hint"] = "text_only"
            n += 1
    # tag (don't delete) broken figures so later metrics can filter them
    figures = doc.get("figures") or {}
    for fid in broken:
        if fid in figures and isinstance(figures[fid], dict):
            figures[fid]["audit_status"] = "broken"
    return n
...
def main() -> int:
    ...
    broken_by_paper = _broken_by_paper(args.audit)
    total_broken = sum(len(v) for v in broken_by_paper.values())
    print(f"[*] {'APPLY' if args.apply else 'DRY-RUN'}: {total_broken} broken figures across "
          f"{len(broken_by_paper)} papers")
    ...
    for stem, broken in sorted(broken_by_paper.items()):
        path = args.cache_dir / f"{stem}.json"
        if not path.exists():
            print(f"  [skip] {stem}: cache file not found")
            continue
        doc = json.loads(path.read_text(encoding="utf-8"))
        n = clean_doc(doc, broken)
        total_panels_changed += n
        print(f"  {stem[:44]:<44}  demote {n} panel(s)  (broken figs: {sorted(broken)})")
        if args.apply:
            path.write_text(json.dumps(doc, ensure_ascii=False, indent=2), encoding="utf-8")

    print("-" * 80)
    print(f"[=] {total_panels_changed} panels demoted to text_only across {len(broken_by_paper)} papers")
    if not args.apply:
